...
namespace Logic
{
    public class MovePlateLogic : MonoBehaviour
    {
        ...
        public void OnMouseUpAsButton()
        {
            ...
            if (_ownerReference.GetComponent<GamePieceLogic>().name == PlayerColour.White.ToString() && _matrixY + _yDir == _boardSize - 1)
            {
                controller.GetComponent<Game>().Winner(PlayerColour.White);
            }
            else if (_ownerReference.GetComponent<GamePieceLogic>().name == PlayerColour.Black.ToString() && _matrixY + _yDir == 0)
            {
